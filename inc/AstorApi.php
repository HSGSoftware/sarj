<?php

namespace App\Api;

class AstorApi
{
    private const GW_BASE   = 'https://api-gw.beefull.io/api/beefull-charge-management';
    private const AUTH      = 'Basic ';
    private const COOKIE    = '_csrf=kjOUNx7kd-66hHglCOuBQjmS';
    private const CACHE_TTL_LIST   = 300;  // 5 dakika - istasyon listesi
    private const CACHE_TTL_DETAIL = 90;   // 90 saniye - soket detayı

    /**
     * Beefull API'sinden tüm astor+beefull istasyonlarını çeker.
     * Dönen format: EPDK stations formatıyla uyumlu
     * (id=beefullId, lat, lng, title, brand, available, green, sockets)
     */
    public function getAllStations(): array
    {
        $cacheKey = 'bf_all_stations';
        $cached   = $this->getFromCache($cacheKey, self::CACHE_TTL_LIST);
        if ($cached !== null) return $cached;

        $url = self::GW_BASE . '/grouped-charge-points-alternative'
             . '?columns=id,coordinate,unitName,tenantCode,availabilityStatusCode,connectorCurrentType,maxPower'
             . '&serviceTypeCode=ev&isVisible=true&tenantCode=beefull,astor&pbAvailabilityStatusCode=available';

        $raw = $this->curl($url, [
            'inavitas-tenant: astor',
            'authorization: ' . self::AUTH,
            'accept: application/json',
            'user-agent: okhttp/4.9.2',
        ]);

        if (!$raw) return [];
        $list = json_decode($raw, true);
        if (!is_array($list)) return [];

        $stations = [];
        foreach ($list as $item) {
            if (!isset($item['coordinate']['latitude'], $item['coordinate']['longitude'])) continue;
            $tenant  = $item['tenantCode'] ?? 'astor';
            $avail   = ($item['availabilityStatusCode'] ?? '') === 'available';
            $type    = strtoupper($item['connectorCurrentType'] ?? 'AC');
            $power   = $item['maxPower'] ?? null;

            $stations[] = [
                'id'        => 'bf_' . $item['id'],   // prefix ile EPDK id'lerinden ayır
                'beefull_id'=> (int)$item['id'],
                'lat'       => (float)$item['coordinate']['latitude'],
                'lng'       => (float)$item['coordinate']['longitude'],
                'title'     => $item['unitName'] ?? '',
                'brand'     => strtoupper($tenant),    // ASTOR veya BEEFULL
                'available' => $avail,
                'green'     => 'HAYIR',
                'sockets'   => $power ? [['type' => $type, 'power' => (float)$power]] : [],
                'source'    => 'beefull',
                'tenantCode'=> $tenant,
            ];
        }

        $this->saveToCache($cacheKey, $stations);
        return $stations;
    }

    /**
     * Beefull ID ile soket detayını çeker (koordinat eşleştirme yok).
     */
    public function getSocketsByBeefullId(int $beefullId, string $tenantCode = 'astor'): ?array
    {
        $cacheKey = 'bf_sockets_' . $beefullId;
        $cached   = $this->getFromCache($cacheKey, self::CACHE_TTL_DETAIL);
        if ($cached !== null) return $cached;

        // Önce tenant ile dene, boşsa diğerini dene
        foreach ([$tenantCode, $tenantCode === 'astor' ? 'beefull' : 'astor', ''] as $tc) {
            $url = self::GW_BASE . '/charge-points-alternative/' . $beefullId;
            if ($tc !== '') $url .= '?tenantCode=' . $tc;

            $raw = $this->curl($url, [
                'inavitas-tenant: astor',
                'authorization: ' . self::AUTH,
                'cookie: '        . self::COOKIE,
                'accept: application/json',
                'cache-control: no-cache',
            ]);

            if (!$raw) continue;
            $data = json_decode($raw, true);
            if (!is_array($data) || empty($data)) continue;

            $children = $data[0]['children'] ?? [];
            if (empty($children)) continue;

            $sockets = $this->normalizeChildren($children, $data[0]);
            $this->saveToCache($cacheKey, $sockets);
            return $sockets;
        }

        return null;
    }

    private function normalizeChildren(array $children, array $parent): array
    {
        $statusMap = [
            'available'   => 'FREE',
            'charging'    => 'CHARGING',
            'finishing'   => 'CHARGING',
            'preparing'   => 'PREPARING',
            'reserved'    => 'RESERVED',
            'faulted'     => 'FAULTED',
            'unavailable' => 'OFFLINE',
            'offline'     => 'OFFLINE',
        ];

        $connectorMap = [
            'ccs2'    => 'DC_CCS2',
            'ccs1'    => 'DC_CCS1',
            'chademo' => 'DC_CHAdeMO',
            'type2'   => 'AC_TYPE2',
            'type1'   => 'AC_TYPE1',
            'schuko'  => 'AC_Schuko',
            'gb/t'    => 'DC_GBT',
        ];

        $result = [];
        foreach ($children as $child) {
            $meta        = $child['metadata'] ?? [];
            $statusRaw   = strtolower($child['availabilityStatusCode'] ?? '');
            $status      = $statusMap[$statusRaw] ?? strtoupper($statusRaw);
            $connType    = strtolower($meta['connectorType'] ?? '');
            $currentType = strtoupper($meta['connectorCurrentType'] ?? 'AC');
            $power       = isset($meta['power']) ? (float)$meta['power'] : null;
            $socRaw      = $meta['SoC'] ?? null;
            $tariff      = $child['tariff'] ?? null;
            $price       = $tariff ? ($tariff['usage']['rate'][0]['value'] ?? null) : null;

            if ($currentType === 'DC' && $connType === 'type2') {
                $subType = 'DC_CCS2';
            } else {
                $subType = isset($connectorMap[$connType]) ? $connectorMap[$connType] : (strtoupper($connType) ?: $currentType);
            }

            $activeStatuses = ['CHARGING', 'PREPARING', 'IN_USE'];
            $soc = (in_array($status, $activeStatuses) && $socRaw !== null && $socRaw !== 'null' && $socRaw !== '') ? (int)$socRaw : null;

            $result[] = [
                'id'          => $child['id'],
                'connectorNo' => $meta['connectorId'] ?? null,
                'type'        => $currentType,
                'subType'     => $subType,
                'socketNumber'=> 'BFLL-' . ($child['id'] ?? '?'),
                'power'       => $power,
                'price'       => $price,
                'currency'    => $tariff ? ($tariff['currencySymbol'] ?? '₺') : '₺',
                'soc'         => $soc,
                'status'      => $status,
                'availability'=> [[
                    'status'    => $status,
                    'startTime' => date('Y-m-d') . 'T00:00:00',
                    'endTime'   => date('Y-m-d') . 'T23:59:59',
                ]],
                'prices' => $price !== null ? [[
                    'price'     => $price,
                    'startTime' => date('Y-m-d') . 'T00:00:00',
                    'endTime'   => date('Y-m-d') . 'T23:59:59',
                ]] : [],
                'source'      => 'beefull',
                'lastUpdate'  => $parent['lastCommunication'] ?? null,
            ];
        }
        return $result;
    }

    private function curl(string $url, array $headers): ?string
    {
        $ch = curl_init();
        curl_setopt_array($ch, [
            CURLOPT_URL            => $url,
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING       => 'gzip',
            CURLOPT_TIMEOUT        => 15,
            CURLOPT_HTTPHEADER     => $headers,
            CURLOPT_SSL_VERIFYPEER => false,
            CURLOPT_SSL_VERIFYHOST => false,
        ]);
        $response = curl_exec($ch);
        $code     = curl_getinfo($ch, CURLINFO_HTTP_CODE);
        curl_close($ch);
        return ($code === 200 && $response) ? $response : null;
    }

    private function getCacheFile(string $key): string
    {
        return sys_get_temp_dir() . '/sarjnet_' . md5($key) . '.json';
    }

    private function getFromCache(string $key, int $ttl)
    {
        $file = $this->getCacheFile($key);
        if (!file_exists($file) || (time() - filemtime($file)) > $ttl) return null;
        $data = file_get_contents($file);
        return $data ? json_decode($data, true) : null;
    }

    private function saveToCache(string $key, $data): void
    {
        file_put_contents($this->getCacheFile($key), json_encode($data, JSON_UNESCAPED_UNICODE));
    }
}
