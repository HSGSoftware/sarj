...
<div class="payment-wrap">
    ...
    <div class="kwh-grid">
        <button class="kwh-opt" data-kwh="10" data-price="45.00"><span class="kwh-opt-amount">10 kWh</span><span class="kwh-opt-price">₺45,00</span></button>
        <button class="kwh-opt active" data-kwh="20" data-price="88.00"><span class="kwh-opt-amount">20 kWh</span><span class="kwh-opt-price">₺88,00</span><span class="kwh-opt-badge">Popüler</span></button>
        <button class="kwh-opt" data-kwh="30" data-price="129.00"><span class="kwh-opt-amount">30 kWh</span><span class="kwh-opt-price">₺129,00</span></button>
        <button class="kwh-opt" data-kwh="50" data-price="210.00"><span class="kwh-opt-amount">50 kWh</span><span class="kwh-opt-price">₺210,00</span></button>
    </div>
    ...
    <div class="form-card">
        ...
        <form id="paymentForm" onsubmit="processPayment(event)">
            <div class="form-group"><label>Kart Sahibi</label><input type="text" id="cardName" placeholder="Ad Soyad" required autocomplete="cc-name"></div>
            <div class="form-group"><label>Kart Numarası</label><input type="text" id="cardNumber" placeholder="0000 0000 0000 0000" maxlength="19" required inputmode="numeric" autocomplete="cc-number"></div>
            <div class="form-row">
                <div class="form-group"><label>Son Kullanma</label><input type="text" id="cardExpiry" placeholder="AA/YY" maxlength="5" required inputmode="numeric"></div>
                <div class="form-group"><label>CVV</label><input type="text" id="cardCvv" placeholder="123" maxlength="4" required inputmode="numeric"></div>
            </div>
            <div class="form-group"><label>E-posta</label><input type="email" id="cardEmail" placeholder="ornek@email.com" inputmode="email"></div>
            <button type="submit" class="btn btn-primary btn-block btn-pay" id="payBtn">
                <i class="fa-solid fa-lock"></i> Güvenli Öde – <span id="payAmount">₺88,00</span>
            </button>
        </form>
    </div>
...
<script>
const stationId=<?= (int)$stationId ?>, stationName=<?= json_encode($stationName) ?>;
...
document.querySelectorAll('.kwh-opt').forEach(btn=>{
    btn.addEventListener('click',()=>{
        ...
        btn.classList.add('active'); selKwh=btn.dataset.kwh; selPrice=btn.dataset.price;
...
function processPayment(e) {
    ...
    setTimeout(()=>{
        ...
        window.location.href=PAGES.payment_success+'?'+p.toString();
    },2000);
}
</script>
