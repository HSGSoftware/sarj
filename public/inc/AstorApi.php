<?php

namespace App\Api;

class AstorApi
{
    private const BASE_URL  = 'https://api-gw.beefull.io/api/beefull-charge-management/charge-points-alternative';
    private const AUTH      = 'Basic ';
    private const CACHE_TTL = 120; // 2 dakika

    /** EPDK station ID → Beefull ID haritası */
    private array $idMap = [];

    public function __construct()
    {
        $mapFile = ROOT_PATH . '/assets/astor_id_map.json';
        if (file_exists($mapFile)) {
            $this->idMap = json_decode(file_get_contents($mapFile), true) ?? [];
        }
    }

    /** EPDK sarjtr ID'sinden Astor Beefull soket verisini çek */
    public function getSocketsByEpdkId(int $epdkId): ?array
    {
        $beefullId = $this->idMap[(string)$epdkId] ?? $this->idMap[$epdkId] ?? null;
        if (!$beefullId) {
            return null;
        }
        return $this->getSocketsByBeefullId((int)$beefullId);
    }

    public function getSocketsByBeefullId(int $beefullId): ?array
    {
        $cacheKey = 'astor_' . $beefullId;
        $cached   = $this->getFromCache($cacheKey);
        if ($cached !== null) {
            return $cached;
        }

        $url = self::BASE_URL . '/' . $beefullId . '?tenantCode=astor';
        $ch  = curl_init();
        curl_setopt_array($ch, [
            CURLOPT_URL            => $url,
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING       => '',
            CURLOPT_TIMEOUT        => 10,
            CURLOPT_HTTP_VERSION   => CURL_HTTP_VERSION_1_1,
            CURLOPT_CUSTOMREQUEST  => 'GET',
            CURLOPT_HTTPHEADER     => [
                'inavitas-tenant: astor',
                'cache-control: no-cache',
                'authorization: ' . self::AUTH,
                'cookie: _csrf=kjOUNx7kd-66hHglCOuBQjmS',
                'accept: application/json',
            ],
            CURLOPT_SSL_VERIFYPEER => false,
            CURLOPT_SSL_VERIFYHOST => false,
        ]);

        $response = curl_exec($ch);
        $err      = curl_error($ch);
        $code     = curl_getinfo($ch, CURLINFO_HTTP_CODE);
        curl_close($ch);

        if ($err || $code !== 200 || !$response) {
            return null;
        }

        $data = json_decode($response, true);
        if (!is_array($data) || empty($data)) {
            return null;
        }

        $station  = $data[0];
        $children = $station['children'] ?? [];
        $sockets  = $this->normalizeChildren($children, $station);

        $this->saveToCache($cacheKey, $sockets);
        return $sockets;
    }

    /** Beefull children → uygulamamızın soket formatına dönüştür */
    private function normalizeChildren(array $children, array $parent): array
    {
        $statusMap = [
            'available'  => 'FREE',
            'charging'   => 'IN_USE',
            'finishing'  => 'IN_USE',
            'preparing'  => 'IN_USE',
            'reserved'   => 'RESERVED',
            'faulted'    => 'FAULTED',
            'unavailable'=> 'OFFLINE',
            'offline'    => 'OFFLINE',
        ];

        $connectorTypeMap = [
            'ccs2'      => 'DC_CCS2',
            'ccs1'      => 'DC_CCS1',
            'chademo'   => 'DC_CHADEMO',
            'type2'     => 'AC_TYPE2',   // AC Type 2
            'type1'     => 'AC_TYPE1',
            'schuko'    => 'AC_SCHUKO',
            'gb/t'      => 'DC_GBT',
            'gbt'       => 'DC_GBT',
        ];

        $result = [];
        foreach ($children as $child) {
            $meta          = $child['metadata'] ?? [];
            $statusCode    = strtolower($child['availabilityStatusCode'] ?? 'unavailable');
            $connType      = strtolower($meta['connectorType'] ?? '');
            $currentType   = strtoupper($meta['connectorCurrentType'] ?? 'AC');
            $power         = isset($meta['power']) ? (float)$meta['power'] : null;
            $tariff        = $child['tariff'] ?? null;
            $price         = $tariff ? ($tariff['usage']['rate'][0]['value'] ?? null) : null;

            // DC ile Type2 connector kombinasyonu = CCS2 olarak düzelt
            if ($currentType === 'DC' && $connType === 'type2') {
                $resolvedSubType = 'DC_CCS2';
            } else {
                $resolvedSubType = $connectorTypeMap[$connType] ?? (strtoupper($connType) ?: $currentType);
            }

            $result[] = [
                'id'             => $child['id'] ?? null,
                'type'           => $currentType,
                'subType'        => $resolvedSubType,
                'socketNumber'   => 'BFLL-' . ($child['id'] ?? '?'),
                'power'          => $power,
                'price'          => $price,
                'currency'       => $tariff['currency'] ?? 'TRY',
                'availability'   => [[
                    'status'    => $statusMap[$statusCode] ?? strtoupper($statusCode),
                    'startTime' => date('Y-m-d') . 'T00:00:00',
                    'endTime'   => date('Y-m-d') . 'T23:59:59',
                ]],
                'prices'         => $price !== null ? [[
                    'price'     => $price,
                    'startTime' => date('Y-m-d') . 'T00:00:00',
                    'endTime'   => date('Y-m-d') . 'T23:59:59',
                ]] : [],
                'source'         => 'astor',
                'lastUpdate'     => $parent['lastCommunication'] ?? null,
            ];
        }
        return $result;
    }

    private function getCacheFile(string $key): string
    {
        return sys_get_temp_dir() . '/sarjnet_' . md5($key) . '.json';
    }

    private function getFromCache(string $key): mixed
    {
        $file = $this->getCacheFile($key);
        if (!file_exists($file) || (time() - filemtime($file)) > self::CACHE_TTL) {
            return null;
        }
        $data = file_get_contents($file);
        return $data ? json_decode($data, true) : null;
    }

    private function saveToCache(string $key, mixed $data): void
    {
        file_put_contents($this->getCacheFile($key), json_encode($data, JSON_UNESCAPED_UNICODE));
    }
}
