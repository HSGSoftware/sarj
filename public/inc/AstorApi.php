...
class AstorApi
{
    ...
    // Koordinat toleransı (metre).
    // EPDK ve Beefull aynı koordinatı kullanır ama float hassasiyeti farkı olabilir.
    // Makul üst sınır: 100m (aynı istasyon farklı sistemlerde max bu kadar kayabiliyor).
    private const COORD_TOLERANCE_M = 100;
    ...
    private function findIdsByCoord(float $lat, float $lng): array
    {
        ...
        // En yakın istasyonu bul, tolerans içindeyse döndür
        ...
        foreach ($list as $item) {
            ...
            $d    = $this->distanceM($lat, $lng, $iLat, $iLng);
            if ($d < $bestDist) {
                $bestDist = $d;
                $bestIds  = [$item['id']];
            } elseif ($d === $bestDist) {
        ...
        return ($bestDist <= self::COORD_TOLERANCE_M) ? $bestIds : [];
    }
